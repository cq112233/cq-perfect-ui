import { getHdNum, getRandomInt } from './../helper/index';
 ...
 * @LastEditors: chen qi
 * @LastEditTime: 2023-03-17 22:48:24
 * @Description: 子弹
 */
export default class Cullet {
  ...
  remove() {
    this.controlled.culletList.splice(this.controlled.culletList.indexOf(this),1)
    this.isAlive = false
  }
  ...
  draw() {
    ...
    this.chart.monsterList.forEach(monster=>{
      ...
      const xControlledTrue = this.controlled.x + this.controlled.radius < monster.x 
    ...
    this.y = this.y
    this.ctx.fillStyle = `rgb(${getRandomInt(0,255)},${getRandomInt(0,255)},${getRandomInt(0,255)})`
